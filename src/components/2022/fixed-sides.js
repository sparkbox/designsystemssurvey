...
const DotLink = ({title, to}) => {
  return (
    <AnchorLink aria-hidden={true} className="cmp-fixed-sides__dot" href={`#section-${to}`} data-indicator={`section-${to}`} aria-labelledby={`section-link-${to}`} data-label={title}>
...
const FixedSides = () => {
  return (
    <div className="cmp-fixed-sides js-section-indicators">
      <AnchorLink className="cmp-fixed-sides__top" href="#top" data-label="Back To Top" aria-label="Back To Top">
